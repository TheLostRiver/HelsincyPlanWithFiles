...
| Automatic takeover | A second conversation starts PWF and silently uses the task owned by a previous conversation | No task ownership gate distinguishes "workspace fallback" from "this session is allowed to use this plan" |
...
- Detect other sessions that are already using PWF in the same project before a new session falls back to workspace state.
...
- Prevent a new or unbound session from automatically taking over another session's PWF task, even if the old session is no longer actively running.
...
- Do not automatically release task ownership just because a Codex conversation stopped, was compacted, or is no longer sending heartbeats.
...
### 4.1 Session Lease and Task Ownership Gate

Session binding answers "which plan does this session want?" It does not by itself answer "is this session allowed to use that plan?" Add a second gate based on lightweight leases:

- A session lease records that a Codex session has used PWF in this project.
- A task lease records which session currently owns a plan.
- Hooks and CLI commands register or refresh the current session lease before resolving context.
- If the current session has no binding and `.planning/.active_plan` points to a task owned by another session, the hook must emit a diagnostic and avoid injecting that task's context or appending to its `progress.md`.
- A stale owner is still an owner. Heartbeat expiry helps diagnostics, but it does not automatically transfer ownership.
- Ownership transfer requires an explicit CLI action such as `--force-claim`; intentional sharing requires an explicit shared-task action such as `--share`.

This makes "stopped running" different from "released." A previous conversation can stop producing hook events, but the next conversation must not silently inherit its PWF task. The safe default is deny-with-diagnostic for conflicting ownership, then ask the user to bind a new task, release the old binding, join a shared task, or force-claim a stale task.
...
Lease state meanings:

| State | Meaning | Can another session auto-use the task? |
|-------|---------|----------------------------------------|
| `active` | Owner heartbeat is within the freshness window | No |
| `stale` | Owner heartbeat expired or the process stopped sending hook events | No |
| `released` | Owner explicitly cleared or released the binding | Yes, after normal validation |
| `shared` | Task was explicitly marked shareable | Yes, but records still include `Session` metadata |

Default heartbeat freshness can start at 10 minutes and be configurable through `PWF_SESSION_LEASE_TTL_SECONDS`. The TTL only changes diagnostics from active to stale; it never grants automatic ownership transfer.
...
- `--session` claims the task lease only when there is no conflicting owner.
- `--force-claim` is required when another session owns the task, even if that owner is stale.
- `--share` marks the task lease as intentionally shared.
...
`--clear-session` removes the current session binding so the conversation falls back to workspace mode. `--release-session` removes the binding and marks the owned task lease as `released` when the current session is the owner.
...
When workspace fallback points at a task owned by another session:
...
- Session leases and task leases prevent unbound sessions from automatically taking over another conversation's task.
- `PLAN_ID` is the strongest explicit override.
- `--session` switch avoids changing other conversations.
- `--force-claim`, `--share`, and `--release-session` are explicit ownership operations.
...
If a second conversation starts PWF and the workspace active task is owned by another session, PWF will not automatically take over that task. Bind the new conversation to its own task, explicitly share the task, or use --force-claim only when you mean to take ownership.
```
...
- Unbound second session does not inject or write to a task owned by another session.
- Stale task owner still blocks automatic takeover.
- Shared task lease allows multiple sessions and preserves `Session` metadata.

### 13.3 Lease and Ownership Tests

- Starting PWF registers or refreshes the current session lease.
- `switch --session` creates a task lease owned by the current session.
- Unbound workspace fallback is denied when `.active_plan` points at another session's task lease.
- Stale leases are reported as stale but still block automatic takeover.
- `switch --session --force-claim` transfers ownership explicitly.
- `switch --session --share` marks a task as intentionally shared.
- `switch --release-session` marks the owned task lease as released.
...
- `switch --session --force-claim` can claim a stale or active task owned by another session only when explicitly requested.
- `switch --session --share` marks the task lease shared.
- `switch --release-session` releases the current session's owned task.
- `switch --workspace` writes `.active_plan`.
- `switch --clear-session` removes the binding file.
- `status` shows workspace active, session binding, session lease, task lease, and effective plan.
- `doctor` warns about multiple sessions sharing workspace fallback and task lease conflicts.
...
- [ ] Add session lease read/write helpers under `.planning/session-leases/`.
- [ ] Add task lease read/write helpers under `.planning/<plan-id>/.task-lease.json`.
- [ ] Refresh the current session lease before hook context resolution.
- [ ] Deny workspace fallback when the effective plan is owned by a different non-shared session.
- [ ] Treat stale task owners as conflicts until explicit `--force-claim` or `--release-session`.
- [ ] Add `--force-claim`, `--share`, and `--release-session` CLI behavior.
- [ ] Add status and doctor diagnostics for session leases, task leases, stale owners, conflicts, and shared tasks.
- [ ] Add hook, CLI, and doctor tests for automatic takeover prevention.
...
- [ ] Document session leases, task ownership, stale owners, `--force-claim`, `--share`, and `--release-session`.
...
2. Add doctor warnings for likely concurrent workspace-sharing cases and task lease conflicts.
3. Keep automatic takeover blocked by default, including stale-owner cases.
4. Collect feedback on `--session`, `--bind-session`, `--force-claim`, `--share`, and `--release-session` ergonomics.
5. Later consider making `/pwf-init` bind automatically when a reliable `session_id` is always available.
6. Later consider making strict mode require bindings by default in a major or clearly announced release.
...
3. What should the default lease TTL be for marking owners stale: 10 minutes, 30 minutes, or configurable only?
4. Should a shared task inject only records from the current session by default, or should shared-task progress remain global?
5. How stable is Codex `session_id` across context compaction, resume, and thread continuation in the desktop app?
6. Should the binding source use `session_id` only, or should it prefer a more stable thread id if Codex exposes one later?

## 17. Success Criteria

- Two conversations in the same project can bind to different plan ids.
- A second unbound conversation cannot automatically use or write to a task owned by another session.
- A stale owner still blocks automatic takeover until explicit release, share, or force-claim.
...
- Tests cover resolver precedence, invalid binding data, task ownership conflicts, stale leases, strict-mode diagnostics, CLI binding commands, and progress metadata.
